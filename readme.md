...
本プロジェクトの目的は、**接続文字列を最小化しつつ、再現性とコスト意識を両立したAzure上のログ生成基盤を検証すること**です。
単に動作するサーバーレス構成ではなく、**コスト構造・監視設計・アクセス制御を含めた全体設計**を意識しています。

### 1-1：背景と課題

実運用では以下のような課題が発生しがちと考えます。

* 接続文字列へ頼ることによるセキュリティリスク
* 手作業による環境構築に起因する構成差異
* 監視ログのデータ取込量増加に伴うコスト増加
* ログ保存と監視設計が分離しており、全体最適が意識されていない

特に、監視基盤のコストは「実行回数」よりも「データ取込量」に依存します。
そのため、ログ出力設計と監視設計を同時に考慮しなければ、長期的な運用コストは最適化できません。
※小規模構成であっても、設計を誤ると将来的に監視コストが想定外に増加する可能性あり

本プロジェクトでは、
**最小権限・IaC・従量課金モデルを前提としたログ基盤を小規模構成で検証し、設計思想を明確化すること**を目的としています。
